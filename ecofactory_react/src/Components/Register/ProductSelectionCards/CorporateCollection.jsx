...
import coorporateCollectionImg from "../../../assets/productSelection/coorporateCollection.png";
import { Box, Checkbox, Divider, FormControlLabel } from "@mui/material";
import { useDispatch, useSelector } from "react-redux";
import { corporateCollection } from "../../../redux/slices/productSelection";
...
export default function CorporateCollection() {
  ...
  const selectedOptions = useSelector(
    (state) => state.productSelection.corporateCollection
  );
  const [expanded, setExpanded] = useState(false);
  const corporateCollectionChoices = [
    "Ubps"
  ];
  const handleExpandClick = () => {
    setExpanded(!expanded);
  };
  const handleCheckboxCorporateCollectionChange = (option) => {
    dispatch(corporateCollection(option));
  };
    ...
    <Card>
      ...
      <Collapse in={expanded} timeout="auto" unmountOnExit>
        ...
        >
          ...
          {corporateCollectionChoices.map((option) => (
            <FormControlLabel
              key={option}
              control={
                <Checkbox
                  checked={selectedOptions.includes(option)}
                  onChange={() => handleCheckboxCorporateCollectionChange(option)}
                  size="small"
                />
              }
              label={
